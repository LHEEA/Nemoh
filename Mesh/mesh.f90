...
! Sauvegarde de la description du maillage et de la partie immergee
  OPEN(10,file=TRIM(ID%ID)//'/mesh/Description_Full.tec')
  ...
  END DO
  CLOSE(10)
  OPEN(10,file=TRIM(ID%ID)//'/mesh/Description_Wetted.tec')
...
! Calcul et sauvegarde de la matrice de raideur hydrostatique
  ...
  OPEN(10,FILE=TRIM(ID%ID)//'/mesh/KH.dat')
  DO i=1,6
  ...
  OPEN(10,FILE=TRIM(ID%ID)//'/mesh/GC_hull.dat')
  WRITE(10,'(3(1X,E14.7))') Gcoque(1),Gcoque(2),Gcoque(3)
  CLOSE(10)
  OPEN(10,FILE=TRIM(ID%ID)//'/mesh/Inertia_hull.dat')
  DO i=1,3
