...
!----------------------------------------------------------------------------
    ...
    INTEGER :: I,K,L,M,N,IJJ,NJJ,IMETH
...
!~ C
      ...
      XGA(L,I)=XG(I)+T1UNX*XG1+T2UNX*YG1+XN(I)*EPSN
      YGA(L,I)=YG(I)+T1UNY*XG1+T2UNY*YG1+YN(I)*EPSN
      ZGA(L,I)=ZG(I)+T1UNZ*XG1+T2UNZ*YG1+ZN(I)*EPSN
      ENDDO
    END DO
      IF(NG.EQ.1)THEN
		  XGA(1,I)=XG(I)
