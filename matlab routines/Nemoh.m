...
fclose(fid);
fid = fopen([dir,'\Nemoh.cal'], 'w'); 
for i=1:n-1
    fprintf(fid, [cell2mat(textline(i)),'\n']);
end
fclose(fid);
fid=fopen([dir,'/input.txt'],'wt');
fprintf(fid,' \n 0 \n');
status=fclose(fid);
...
nw=fscanf(fid,'%g',1);
fclose(fid);
fid=fopen([dir,'\Results\ExcitationForce.tec'],'r');
ligne=fgetl(fid)
for c=1:6*nBodies
    ligne=fgetl(fid)
end;
...
for k=1:nw
    ...
    w(i)=ligne(1);
    for j=1:6*nBodies
        Famp(k,j)=ligne(2*j+1);
        Fphi(k,j)=ligne(2*j+1);
    end;
end;
status=fclose(fid);
fid=fopen([dir,'\Results\RadiationCoefficients.tec'],'r');
ligne=fgetl(fid);
for i=1:6*nBodies
    ligne=fgetl(fid);
end;
