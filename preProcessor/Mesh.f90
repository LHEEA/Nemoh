...
#define DIRECTORY FILE
    ...
    INTEGER :: i,j,c,d
    INTEGER :: Npoints,Npanels,Nbodies
    CHARACTER(LEN=80) :: meshfile,line
    INTEGER :: lfile
    INTEGER :: M,N
    ...
    DO i = 1,Mesh%Npanels
      ...
      U(:) = Mesh%X(:,Mesh%P(2,i))-Mesh%X(:,Mesh%P(1,i))
      V(:) = Mesh%X(:,Mesh%P(4,i))-Mesh%X(:,Mesh%P(2,i))
      ...
      ! Center of mass
      Mesh%XM(:,i) =  &
        (Mesh%X(:,Mesh%P(1,i))+Mesh%X(:,Mesh%P(2,i))+Mesh%X(:,Mesh%P(4,i)))/3*A1/Mesh%A(i) + &
        (Mesh%X(:,Mesh%P(2,i))+Mesh%X(:,Mesh%P(3,i))+Mesh%X(:,Mesh%P(4,i)))/3*A2/Mesh%A(i)
    ...
    ! Export mesh
    INQUIRE (DIRECTORY=TRIM(ID%ID)//'/mesh', EXIST=ex)
    IF (.NOT.ex) M=SYSTEM('mkdir '//TRIM(ID%ID)//'/mesh')
    ...
    WRITE(10,*) 0
    CLOSE(10)
    OPEN(10,file=TRIM(ID%ID)//'/mesh/Mesh.tec')
